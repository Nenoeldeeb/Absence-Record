...
class ExportStudentsUseCaseTest {
    ...
    fun `should successfully export selected students`() =
        runTest {
            ...
            every { serializationService.encodeToString<List<StudentExportData>>(any(), any()) } returns
                Result.success(expectedJson)
            every { storageRepository.writeTextToUri(uriString, expectedJson) } returns
                flowOf(Result.success(Unit))

            // Act
            val result = useCase(uriString, selectedIds, allStudents).first()
    ...
    fun `should fail if no students selected`() =
        runTest {
            // Arrange
            val result = useCase("uri", emptySet(), emptyList()).first()
    ...
    fun `should handle empty attendance history`() =
        runTest {
            ...
            every { serializationService.encodeToString<List<StudentExportData>>(any(), any()) } returns
                Result.success("json")
            every { storageRepository.writeTextToUri(any(), any()) } returns
                flowOf(Result.success(Unit))
    ...
    fun `should handle serialization failure`() =
        runTest {
            ...
            // Act
            val result = useCase("uri", selectedIds, allStudents).first()
    ...
    fun `should handle exception during data gathering`() =
        runTest {
            ...
            every { attendanceRepository.getStudentAttendanceDates(1) } throws
